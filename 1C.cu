...
__device__ __global__ void Mean1c(float *,float *);
...
int main(void){
	...
	dim3 dimGrid(Grid,1);
	dim3 dimBlock1(Block/2,1);
	dim3 dimGrid1(1,1);
	dim3 dimBlock2(256,8,4);
	dim3 dimGrid2(1,1);
	for(int i=0;i<loop;i++){
		Mean1c<<<dimGrid,dimBlock>>>(Ad+(i*Width),Bd+(i*Grid));
	}
...
__device__ __global__ void Mean1c(float *Ad,float *Bd){
	int tx=threadIdx.x;
	int bx=blockIdx.x;
...
__device__ __global__ void Std1(float *Ad,float*Bd){
	__device__ __shared__ float As[Total/Block];
	int tx=threadIdx.x;
	int ty=threadIdx.y;
	int tz=threadIdx.z;
	Ad[ty+tz*32]=Ad[ty+tz*4096];
	As[tx]=Ad[tx];
		if(tx%8==0){
